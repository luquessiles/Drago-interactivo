...
<html lang="en">
  <head>
    ...
    <title>Dragon-ARABE7</title>
  </head>
  <body>
    <div id="color-controls">
      <label for="eyeColor">Ojos
        <input type="color" id="eyeColor" value="#ff0000" class="color-btn">
      </label>
      <label for="headColor">Cabeza
        <input type="color" id="headColor" value="#00ff00" class="color-btn">
      </label>
      <label for="bodyColor">Cuerpo
        <input type="color" id="bodyColor" value="#0000ff" class="color-btn">
      </label>
      <label for="wingColor">Alas
        <input type="color" id="wingColor" value="#cccccc" class="color-btn">
      </label>
    </div>
    <svg>
      <defs>
...
// Obtener los selectores de color
...
const wingColorInput = document.getElementById("wingColor");

// Función para cambiar colores en el SVG
function cambiarColores() {
		// Cambiar color de ojos y cabeza
		const cabeza = document.querySelector('g#Cabeza');
		if (cabeza) {
			const paths = cabeza.querySelectorAll('path');
			if (paths.length > 1) {
				paths[1].setAttribute('style', `fill:${eyeColorInput.value};fill-opacity:1`);
			}
			if (paths.length > 0) {
				paths[0].setAttribute('style', `fill:${headColorInput.value};fill-opacity:1`);
			}
		}
		// Cuerpo
		const cuerpo = document.querySelectorAll('g#Espina path');
		cuerpo.forEach(p => {
			p.setAttribute('style', `fill:${bodyColorInput.value};fill-opacity:1`);
		});
		// Alas
		const alas = document.querySelectorAll('g#Aletas path');
		alas.forEach(p => {
			p.setAttribute('style', `fill:${wingColorInput.value};fill-opacity:1`);
		});
}

eyeColorInput.addEventListener('input', cambiarColores);
headColorInput.addEventListener('input', cambiarColores);
bodyColorInput.addEventListener('input', cambiarColores);
wingColorInput.addEventListener('input', cambiarColores);

window.addEventListener('DOMContentLoaded', cambiarColores);
